  ...
  <div id="app">
    <div class="content">
      ...
      <div class="company-content inner-box">
        ...
        <van-row class="p-line">
          <van-col span="9"><p>{{$t('coMe.positionCategories')}}</p></van-col>
          <van-col span="13"><p class="right-van">
...
<script>
  import {getJobsAdd,getJobNature,getEducation,getAge,getExperience,getWage,geTag,postJobsAdd,getJob} from '@/server/getData';
  ...
  export default {
    ...
    data() {
      return {
        title: this.$t('common.postPosition'),
    ...
    methods: {
      ...
      post(){
        ...
        if(nameStr == ''){
          ...
          postJobsAdd(this.data).then(res=>{
            //console.log(res);
            if(res.data == ''){
              Toast(this.$t(`message.` + res.message));
              setTimeout(()=>{
